#Construct model for nooil allsex
# Preamble ----------------------------------------------------------------
...
out.path <- "./Data_outputs_nooil_allsex/"
...
#save the null model
save(m1, file = "Data_outputs_nooil_allsex/Mod_1.rda")
...
#Load null model
load("Data_outputs_nooil_allsex/Mod_1.rda")
## Set up personality formulae 

formula <- list()	

formula[[2]] <- ~ rad_log_5+sex
formula[[3]] <- ~ rad_log_5+fraction+sex
formula[[4]] <- ~ rad_log_5*fraction+sex
...
for (i in 2:length(formula)) {
  ...
  m.list[[i]] <- fitHMM(data=dat, nbStates=3,
                        ...
                        formula = formula[[i]])
  model <- m.list[[i]]
  file.out <- paste0("./Data_outputs_nooil_allsex/", paste0("Mod_", i, ".rda"))
...
for (i in 1:length(formula)) {
  ...
  file.in <- paste0("./Data_outputs_nooil_allsex/", paste0("Mod_", i, ".rda"))
  load(file = file.in)
...
# Print out AIC table
out.path <- "./Data_outputs_nooil_allsex/AIC_table_sex.csv"
write.csv(all.out, out.path, row.names=T)

#####################################################################################
###########################################################################################
#Construct model for with oil allsex
# Preamble ----------------------------------------------------------------
...
### Create outputs folder if not exist already
out.path <- "./Data_outputs_withoil_allsex/"
if(dir.exists(out.path) == FALSE){
  dir.create(out.path)
}
...
#save the null model
save(m1, file = "Data_outputs_withoil_allsex/Mod_1.rda")
...
#Load null model
load("Data_outputs_withoil_allsex/Mod_1.rda")
## Set up personality formulae 
...
for (i in 2:length(formula)) {
  ...
  m.list[[i]] <- fitHMM(data=dat, nbStates=3,
                        ...
                        formula = formula[[i]])
  model <- m.list[[i]]
  file.out <- paste0("./Data_outputs_withoil_allsex/", paste0("Mod_", i, ".rda"))
  save(model, file = file.out)
}
...
for (i in 1:length(formula)) {
  ...
  file.in <- paste0("./Data_outputs_withoil_allsex/", paste0("Mod_", i, ".rda"))
  load(file = file.in)
  if (i == 1) { m.list[[i]] <- m1} else { m.list[[i]] <- model}
...
# Print out AIC table
out.path <- "./Data_outputs_withoil_allsex/AIC_table_sex.csv"
write.csv(all.out, out.path, row.names=T)
