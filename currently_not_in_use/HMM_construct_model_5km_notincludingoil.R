...
out.path <- "./Data_outputs_notincludeOil/"
...
#save the null model
save(m1_M, file = "Data_outputs_notincludeOil/M_mod_1.rda")
save(m1_F, file = "Data_outputs_notincludeOil/F_mod_1.rda")
...
#Load null model
load("Data_outputs_notincludeOil/M_mod_1.rda")
load("Data_outputs_notincludeOil/F_mod_1.rda")
...
for (i in 2:length(formula)) {
  ...
  m.list[[i]] <- fitHMM(data=dat_female, nbStates=3,
                        ...
                        formula = formula[[i]])
  model <- m.list[[i]]
  file.out <- paste0("./Data_outputs_notincludeOil/", paste0("F_mod_", i, ".rda"))
...
for (i in 2:length(formula)) {
  ...
  m.list[[i]] <- fitHMM(data=dat_male, nbStates=3,
                        ...
                        formula = formula[[i]])
  model <- m.list[[i]]
  file.out <- paste0("./Data_outputs_notincludeOil/", paste0("M_mod_", i, ".rda"))
  save(model, file = file.out)
}
...
for (i in 1:length(formula)) {
  ...
  file.in <- paste0("./Data_outputs_notincludeOil/", paste0("F_mod_", i, ".rda"))
  load(file = file.in)
...
# Print out AIC table
out.path <- "./Data_outputs_notincludeOil/AIC_table_F.csv"
...
for (i in 1:length(formula)) {
  ...
  file.in <- paste0("./Data_outputs_notincludeOil/", paste0("M_mod_", i, ".rda"))
  load(file = file.in)
...
# Print out AIC table
out.path <- "./Data_outputs_notincludeOil/AIC_table_M.csv"
write.csv(all.out, out.path, row.names=T)
