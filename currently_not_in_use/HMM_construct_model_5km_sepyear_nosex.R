...
out.path <- "./Data_outputs_sepyear_nosex/"
...
dat1<-subset(dat,NBYear=="NB2223")
dat2<-subset(dat,NBYear=="NB2324")
dat1 <- subset(dat1, step < 90000)
dat1 <- subset(dat1,period=="night")
dat2 <- subset(dat2, step < 90000)
dat2 <- subset(dat2,period=="night")
#hist(dat$step)
...
F_m1 <- fitHMM(data=dat1, nbStates=3,
             dist=list(step="gamma",angle="vm"),
             Par0=list(step=stepPar0, angle=anglePar0),
             estAngleMean = list(angle=TRUE),
             stateNames = stateNames)
S_m1 <- fitHMM(data=dat2, nbStates=3,
               dist=list(step="gamma",angle="vm"),
               Par0=list(step=stepPar0, angle=anglePar0),
               estAngleMean = list(angle=TRUE),
               stateNames = stateNames)
...
#save the null model
save(F_m1, file = "Data_outputs_sepyear_nosex/F_Mod_1.rda")
save(S_m1, file = "Data_outputs_sepyear_nosex/S_Mod_1.rda")
...
#Load null model
#load("Data_outputs_sepyear_nosex/F_Mod_1.rda")
#load("Data_outputs_sepyear_nosex/S_Mod_1.rda")
## Set up personality formulae 

formula <- list()	

formula[[2]] <- ~ rad_log_5*fraction+sex
...
for (i in 2:length(formula)){
  Par[[i]] <- getPar0(model=F_m1, nbStates=3, formula = formula[[i]])
}
...
for (i in 2:length(formula)) {
  print(i)
  m.list[[i]] <- fitHMM(data=dat1, nbStates=3,
                        dist=list(step="gamma",angle="vm"),
                        ...
                        formula = formula[[i]])
  model <- m.list[[i]]
  file.out <- paste0("./Data_outputs_sepyear_nosex/", paste0("F_Mod_", i, ".rda"))
...
Par <- list()
for (i in 2:length(formula)){
  Par[[i]] <- getPar0(model=S_m1, nbStates=3, formula = formula[[i]])
}
...
for (i in 2:length(formula)) {
  print(i)
  m.list[[i]] <- fitHMM(data=dat2, nbStates=3,
                        dist=list(step="gamma",angle="vm"),
                        Par0=list(step=Par[[i]]$Par$step, angle=Par[[i]]$Par$angle,delta0 = Par[[i]]$delta),
                        estAngleMean = list(angle=TRUE), beta0 = Par[[i]]$beta,
                        stateNames = stateNames, 
                        formula = formula[[i]])
  model <- m.list[[i]]
  file.out <- paste0("./Data_outputs_sepyear_nosex/", paste0("S_Mod_", i, ".rda"))
  save(model, file = file.out)
}
# FIND THE BEST MODEL -----------------------------------------------------
...
for (i in 1:length(formula)) {
  ...
  file.in <- paste0("./Data_outputs_sepyear_nosex/", paste0("F_Mod_", i, ".rda"))
  load(file = file.in)
  if (i == 1) { m.list[[i]] <- F_m1} else { m.list[[i]] <- model}
...
# Print out AIC table
out.path <- "./Data_outputs_sepyear_nosex/AIC_table_Fsex.csv"
...
for (i in 1:length(formula)) {
  ...
  file.in <- paste0("./Data_outputs_sepyear_nosex/", paste0("S_Mod_", i, ".rda"))
  load(file = file.in)
  if (i == 1) { m.list[[i]] <- F_m1} else { m.list[[i]] <- model}
...
# Print out AIC table
out.path <- "./Data_outputs_sepyear_nosex/AIC_table_Ssex.csv"
write.csv(all.out, out.path, row.names=T)
