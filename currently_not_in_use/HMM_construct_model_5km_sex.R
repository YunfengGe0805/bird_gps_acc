...
#save the null model
save(m1, file = "Data_outputs_sex/Mod_1.rda")
...
#Load null model
load("Data_outputs_sex/Mod_1.rda")
...
for (i in 2:length(formula)) {
  ...
  m.list[[i]] <- fitHMM(data=dat, nbStates=3,
                        ...
                        formula = formula[[i]])
  model <- m.list[[i]]
  file.out <- paste0("./Data_outputs_sex/", paste0("Mod_", i, ".rda"))
...
for (i in 1:length(formula)) {
  ...
  file.in <- paste0("./Data_outputs_sex/", paste0("Mod_", i, ".rda"))
  load(file = file.in)
...
# Print out AIC table
out.path <- "./Data_outputs_sex/AIC_table_sex.csv"
